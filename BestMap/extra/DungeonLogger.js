...
const exclusions = ["Entrance", "Fairy", "Blood"]
const traps = ["New", "Old"]
const defaultFile = {"dungeons":[]}

const getRoomID = (roomName) => roomsJson.filter(a => a.name == roomName)[0]?.roomID
const getRoomFromID = (roomID) => roomsJson.filter(a => a.roomID == roomID)[0]
...
register("tick", () => {
    ...
    const witherDoors = doors.filter(a => a.type == DoorTypes.WITHER).length - 1

    if (!Dungeon.floor || witherDoors < 1 || (Dungeon.floorNumber >= 3)) return
    
    logged = true
    let server = getServerID()
    if (!server || server == dmapData.lastLogServer) return
    let trapType = -1
    if (trapRoom) trapType = trapRoom.name == "New Trap" ? 0 : 1

    dmapData.lastLogServer = server
    dmapData.save()
    let thisLog = {
        "f": Dungeon.floor, // Floor
        "s": DmapDungeon.dungeonMap.secrets, // Secrets
        "wd": witherDoors, // Wither Doors
        "r": [...new Set(rooms.filter(a => ![RoomTypes.PUZZLE, RoomTypes.YELLOW, RoomTypes.TRAP].includes(a.type)).map(b => b.name).filter(c => !exclusions.includes(c)))], // Rooms
        "p": rooms.filter(a => a.type == "puzzle").map(b => b.name), // Puzzles
        "t": trapType
    }
...
register("command", (floor) => {
    // Thread because this can take a second or two to process with thousands of runs
    new Thread(() => {
        ...
        const sortFrequency = (flatArray) => {
            ...
            for (let v of flatArray) {
                if (!Object.keys(counts).includes(v)) counts[v] = 0
                counts[v]++
            }
